...
def test_static_dashboard_uses_dark_studio_workspace_design() -> None:
    ...
    assert "2026-06-30-dark-admin-ui" in index_html
...
def test_dashboard_sidebar_owns_brand_and_profile_chrome() -> None:
    ...
    assert 'brandMarkup(true, { showFallbackMark: false })' in app_js
    assert ".brand.text-only" in styles_css
...
def test_dashboard_contains_users_management_page() -> None:
    ...
    assert "handleUserDelete" in app_js
    assert "renderUserEditDialog" in app_js
    assert "renderRoleSelect" in app_js
    assert 'data-action="edit-user"' in app_js
    assert 'data-action="delete-user"' in app_js
    ...
    assert "users-table-panel" in styles_css
...
def test_static_app_contains_public_hosted_auth_pages() -> None:
    ...
    assert '"Hosted auth"' not in app_js
    assert "escapeHtml(eyebrow)" not in app_js
